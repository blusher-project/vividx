...
static sk_sp<SkShader> make_fuzz_shader(Fuzz* fuzz, int depth) {
    ...
    switch (shaderType) {
        ...
        default:
            SkDEBUGFAILF("Unknown shader type %d", shaderType);
            break;
    }
    return nullptr;
}
...
void FuzzPaint(Fuzz* fuzz, SkPaint* paint, int depth) {
    if (!fuzz || !paint || depth <= 0 || fuzz->exhausted()) {
        return;
    }
...
static SkTextEncoding FuzzPaint_text_encoding(Fuzz* fuzz) {
...
static sk_sp<SkTextBlob> make_fuzz_textblob(Fuzz* fuzz) {
    ...
    while (runCount-- > 0) {
        ...
        SkTextEncoding encoding = FuzzPaint_text_encoding(fuzz);
...
void FuzzCanvas(Fuzz* fuzz, SkCanvas* canvas, int depth) {
    if (!fuzz || !canvas || depth <= 0 || fuzz->exhausted()) {
        return;
    }
...
static sk_sp<SkPicture> make_fuzz_picture(Fuzz* fuzz, int depth) {
    ...
    FuzzCanvas(fuzz, pictureRecorder.beginRecording(w, h), depth - 1);

...
SkRecordCanvas::SkRecordCanvas(SkRecord* record, const SkRect& bounds)
...
template <typename T, typename... Args> void SkRecordCanvas::append(Args&&... args) {
...
template <typename T> T* SkRecordCanvas::copy(const T* src) {
    if (nullptr == src) {
        return nullptr;
    }
...
template <typename T> T* SkRecordCanvas::copy(const T src[], size_t count) {
    if (nullptr == src) {
        return nullptr;
    }
